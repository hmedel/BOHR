...
# APÉNDICE A — Historial de versiones del criterio E5

## A.1 Definición del criterio

**E5** excluye mensajes que no contienen terminología identificable del curso *Estructura de la Materia* (química cuántica y estructura atómico-molecular, FESC-UNAM). El criterio se aplica sobre texto normalizado —NFD sin diacríticos, minúsculas, sin puntuación— para evitar que caracteres acentuados interrumpan la coincidencia de subcadenas (`cuant` captura `cuántico` una vez normalizado).
...
- Enlace químico: enlace (iónico, covalente), electronegatividad, momento dipolar, Van der Waals, puentes de hidrógeno, ácido, base (Brønsted, Lewis, Arrhenius), anfiprótico, poliprótico.
- Estructura molecular: geometría molecular, VSEPR, hibridación.
- Termodinámica básica: entalpía, entropía, Gibbs, energía.
...
**Cifras exactas del export v2.9.10 (2026-08-14), corpus bruto = 357 mensajes de usuario:**

| Criterio | Excluidos | Acumulado excluido | Restantes |
|----------|----------:|-------------------:|----------:|
| Corpus bruto (role=user) | — | 0 | 357 |
| **E0** (cuentas admin) | **122** | **122** | **235** |
| E1 (< 15 caracteres) | 15 | 137 | 220 |
| E2 (saludo/despedida) | 0 | 137 | 220 |
| E3 (interacción examen) | 0 | 137 | 220 |
| E4-same (dup. mismo usuario) | 4 | 141 | 216 |
| E4-cross (dup. inter-usuario) | 33 | 174 | 183 |
| **E5** (sin química) | **12** | **186** | **171** |
| **Elegibles** | — | — | **171** |
...
Para cada ítem en `auditoria_e5.csv`, el coordinador (u otro docente del curso) completa:
...
El mismo contenido recibe tratamiento opuesto según venga solo o empaquetado con contenido químico. Esta es una limitación estructural de cualquier criterio léxico aplicado a consultas multiparte; no se resuelve añadiendo términos. En el corpus v2.9.10 hay **1 ítem de este tipo** (M369) sobre 171 elegibles (0.6 %; sobre la muestra principal de 146: 0.7 %). El ítem se codifica normalmente; el codificador debe clasificar el nivel más alto de los subcomponentes (R5 del protocolo), independientemente de que uno de ellos caiga fuera del scope de E5.

### A.3.4 Declaración de uso de IA en decisiones de scope

Las decisiones de reincorporación de v4 a v5 (antimateria y superíndice/subíndice) fueron evaluadas con apoyo de un modelo de lenguaje de la familia Claude (Anthropic). El modelo específico y su versión deben verificarse en el historial de la conversación de desarrollo; no se reproduce aquí porque el modelo puede cambiar dentro de una misma conversación y la atribución incorrecta en Agradecimientos es un error de trazabilidad. El juicio de scope se alcanzó en diálogo con ese modelo, no a partir de una revisión independiente de un docente del curso sobre `auditoria_e5.csv`.
...
Para la sección de metodología del artículo se reportará:
